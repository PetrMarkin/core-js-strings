...
function removeFirstOccurrences(str, value) {
  ...
  return str.substring(value, index) + str.substring(index + value.length);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if ('AEIOUYaeiouy'.includes(str[i])) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  const s2 = s1.split('');
  const s3 = s1.split('');
  return s3.reverse().join() === s2.join();
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').sort(function s(a, b) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str.charCodeAt(i) < 78 && str.charCodeAt(i) > 65) {
      ...
      res += String.fromCharCode(str.charCodeAt(i) - 13);
    } else if (str.charCodeAt(i) < 110 && str.charCodeAt(i) > 96) {
      res += String.fromCharCode(str.charCodeAt(i) + 13);
    } else if (str.charCodeAt(i) > 109) {
      res += String.fromCharCode(str.charCodeAt(i) - 13);
    } else if (str[i] === 'A') {
      res += 'N';
    } else {
